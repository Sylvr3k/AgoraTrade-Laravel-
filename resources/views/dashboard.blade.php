    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Roboto', sans-serif; }
        ...
        .sidebar-brand {
            ...
            padding: 24px 24px 20px; border-bottom: 1px solid #2e2e2e;
        }
        ...
        .sidebar-user {
            ...
            padding: 16px 24px; border-bottom: 1px solid #2e2e2e;
        }
        .sidebar-avatar {
            width: 38px; height: 38px; border-radius: 50%;
            object-fit: cover; border: 2px solid #ffe0a9;
        }
        ...
        .nav-section-label {
            ...
            letter-spacing: 1.5px; color: #555; padding: 12px 24px 6px;
        }
        .nav-item {
            display: flex; align-items: center; gap: 12px;
            padding: 11px 24px; color: #aaa; text-decoration: none;
        ...
        .sidebar-footer {
            padding: 16px 24px; border-top: 1px solid #2e2e2e;
        ...
        .gold-btn {
            padding: 10px 22px; background: #ffe0a9; border: none;
            border-radius: 15px 0 15px 0; font-size: 13px; font-weight: bold;
            color: #555; cursor: pointer; text-decoration: none;
            display: inline-block; transition: background 0.2s;
        }
        .gold-btn:hover { background: #f7d9a3; color: #333; }
        .content { padding: 28px 32px; flex: 1; }

        .stats-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 24px; }
        .stat-card { background: #fff; border-radius: 15px 0 15px 0; padding: 20px; border: 1px solid #eee; }
        .stat-icon {
            width: 38px; height: 38px; border-radius: 10px; background: #ffe0a9;
        ...
        .card { background: #fff; border-radius: 15px 0 15px 0; padding: 20px; border: 1px solid #eee; }
        .card-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; }
        .card-title { font-size: 15px; font-weight: bold; color: #333; }

        .chart-outer { display: flex; flex-direction: column; }
        .chart-wrap { display: flex; align-items: flex-end; gap: 5px; height: 140px; }
        .chart-col { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: flex-end; height: 100%; }
        .chart-bar { width: 100%; border-radius: 4px 4px 0 0; min-height: 4px; }
        .chart-bar.gold { background: #ffe0a9; }
        .chart-bar.dark { background: #1a1a1a; }
        .chart-labels { display: flex; gap: 5px; margin-top: 6px; }
        .chart-month { flex: 1; font-size: 9px; color: #bbb; text-align: center; }

        .bar-row { display: flex; align-items: center; gap: 10px; margin-bottom: 14px; }
        .bar-row:last-child { margin-bottom: 0; }
        .bar-label { font-size: 12px; color: #777; width: 90px; flex-shrink: 0; }
        .bar-track { flex: 1; background: #f0f0f0; border-radius: 4px; height: 8px; overflow: hidden; }
        .bar-fill { height: 100%; background: #ffe0a9; border-radius: 4px; }
        .bar-val { font-size: 12px; color: #999; width: 32px; text-align: right; }

        .bottom-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }

        .orders-table { width: 100%; border-collapse: collapse; }
        .orders-table th { font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; color: #999; padding: 8px 10px; text-align: left; border-bottom: 1px solid #f0f0f0; }
        .orders-table td { padding: 11px 10px; font-size: 13px; color: #333; border-bottom: 1px solid #f5f5f5; }
        .orders-table tr:last-child td { border-bottom: none; }
        .orders-table tr:hover td { background: #fafafa; }

        .avatar { width: 30px; height: 30px; border-radius: 50%; background: #ffe0a9; display: inline-flex; align-items: center; justify-content: center; font-size: 11px; font-weight: bold; color: #555; margin-right: 6px; vertical-align: middle; }
        .badge { font-size: 10px; padding: 3px 10px; border-radius: 4px; font-weight: bold; }
        .badge-success { background: #EAF3DE; color: #3B6D11; }
        .badge-warn    { background: #FAEEDA; color: #854F0B; }
        .badge-info    { background: #E6F1FB; color: #185FA5; }

        .quick-link { display: flex; align-items: center; gap: 12px; padding: 12px; border-radius: 15px 0 15px 0; border: 1px solid #eee; text-decoration: none; margin-bottom: 10px; transition: background 0.2s; }
        .quick-link:last-child { margin-bottom: 0; }
        .quick-link:hover { background: #f8f9fa; }
        .ql-icon { width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-size: 16px; }
        .ql-gold { background: #ffe0a9; }
        .ql-dark { background: #1a1a1a; }
        .ql-gray { background: #f0f0f0; }
        .ql-text { font-size: 13px; font-weight: bold; color: #333; }
        .ql-sub  { font-size: 11px; color: #999; }
        ...
        <div class="bottom-grid">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ url('/orders') }}" style="font-size:12px;color:#999;text-decoration:none;">View all →</a>
                </div>
                ...
                <table class="orders-table">
                    <thead>
                        <tr><th>Buyer</th><th>Item</th><th>Price</th><th>Status</th></tr>
                    </thead>
                    <tbody>
                        @foreach($recentOrders as $order)
                        <tr>
                            <td><span class="avatar">{{ strtoupper(substr($order->buyer_name, 0, 1)) }}</span>{{ $order->buyer_name }}</td>
                            <td>{{ $order->item_name }}</td>
                            <td>${{ number_format($order->price, 2) }}</td>
                            <td><span class="badge {{ $order->status === 'Delivered' ? 'badge-success' : ($order->status === 'Pending' ? 'badge-warn' : 'badge-info') }}">{{ $order->status }}</span></td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="card">
                ...
                <a href="{{ url('/listings/create') }}" class="quick-link">
                    ...
                    <div><div class="ql-text">Add New Listing</div><div class="ql-sub">Post an item for sale</div></div>
                </a>
                <a href="{{ url('/listings') }}" class="quick-link">
                    <div class="ql-icon ql-gray">📋</div>
                    <div><div class="ql-text">My Listings</div><div class="ql-sub">{{ $activeListings }} active items</div></div>
                </a>
                <a href="{{ url('/orders') }}" class="quick-link">
                    <div class="ql-icon ql-dark">🚚</div>
                    <div><div class="ql-text">View Orders</div><div class="ql-sub">{{ $pendingOrders }} pending dispatch</div></div>
                </a>
                <a href="{{ url('/purchases') }}" class="quick-link">
                    <div class="ql-icon ql-gold">🛒</div>
                    <div><div class="ql-text">My Purchases</div><div class="ql-sub">Items you've bought</div></div>
                </a>
            </div>
        </div>
