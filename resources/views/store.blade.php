...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Roboto', sans-serif; }
        ...
        /* ── Store layout ── */
        .store-wrap { max-width: 1300px; margin: 0 auto; padding: 30px 20px; }
        ...
        .store-hero h1 { font-size: 28px; font-weight: bold; color: #fff; }
        .store-hero p { font-size: 14px; color: #888; margin-top: 6px; }
        .store-hero-stat { text-align: center; }
        .store-hero-stat-value { font-size: 28px; font-weight: bold; color: #ffe0a9; }
        .store-hero-stat-label { font-size: 12px; color: #888; margin-top: 2px; }
        ...
        .filter-btn {
            padding: 7px 16px; border: 1px solid #ddd;
            border-radius: 15px 0 15px 0; font-size: 12px; font-weight: bold;
            background: #fff; color: #777; cursor: pointer; transition: all 0.2s;
            text-decoration: none; display: inline-block;
        }
        .filter-btn:hover { background: #1a1a1a; color: #fff; border-color: #1a1a1a; }
        .filter-btn.active { background: #ffe0a9; color: #555; border-color: #ffe0a9; }
        ...
        .sort-select {
            ...
            border-radius: 15px 0 15px 0; font-size: 13px;
            outline: none; background: #fff; color: #555; cursor: pointer;
        }
        .sort-select:focus { border-color: #ffe0a9;}
        ...
        .product-card:hover { box-shadow: 0 6px 20px rgba(0,0,0,0.08); transform: translateY(-3px); }
        ...
        .product-name { font-size: 14px; font-weight: 500; color: #333; margin-bottom: 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .product-meta { display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; }
        .product-price { font-size: 18px; font-weight: bold; color: #333; }
        .product-category { font-size: 11px; color: #999; }
        .product-seller { font-size: 11px; color: #999; margin-bottom: 12px; }
        .product-seller span { color: #555; font-weight: 500; }
        ...
        .buy-btn.sold { background: #ddd; color: #999; cursor: not-allowed; }
        ...
        .buy-modal-form label {
            ...
            font-weight: bold; color: #555; margin-bottom: 6px;
        }
        ...
        .buy-modal-confirm-btn:hover { background: #f7d9a3; }
        ...
        <div class="product-card">
            ...
            <div class="product-info">
                ...
                <div class="product-meta">
                    <div class="product-price">${{ number_format($listing->price, 2) }}</div>
                ...
                @else
                    <a href="{{ url('/login') }}" class="buy-btn" style="display:block; text-align:center; text-decoration:none;">Login to Buy</a>
                @endif
            </div>
        </div>
        @empty
...
<div class="modal-overlay" id="buyModal" onclick="closeModalOutside(event)">
    <div class="buy-modal">
        ...
        <form method="POST" action="{{ url('/orders') }}" class="buy-modal-form" id="mpesaForm">
            ...
            <input type="text" name="buyer_name" id="modal-buyer-name" value="{{ Auth::user()->fullname ?? '' }}" required>

            <label>Email</label>
            <input type="email" name="buyer_email" id="modal-buyer-email" value="{{ Auth::user()->email ?? '' }}" required>

            <label>M-Pesa Phone Number</label>
            <input type="text" name="phone" id="modal-phone" placeholder="e.g. 0712345678" required>

            <label>Shipping Address</label>
            <textarea name="buyer_address" id="modal-buyer-address" placeholder="Street, City, Country" required></textarea>

            <div id="mpesa-message" style="display:none; padding:10px; border-radius:15px 0 15px 0; margin-bottom:14px; font-size:13px; text-align:center;"></div>

            <button type="submit" class="buy-modal-confirm-btn" id="mpesa-btn">📱 Pay with M-Pesa</button><br/><br/>
