...
<head>
    ...
    <title>Purchases — AgoraTrade</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Roboto', sans-serif; }
        ...
        .card { background: #fff; border-radius: 15px 0 15px 0; border: 1px solid #eee; overflow: hidden; }

        .purchases-table { width: 100%; border-collapse: collapse; }
        .purchases-table th { font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; color: #999; padding: 12px 16px; text-align: left; border-bottom: 1px solid #f0f0f0; background: #fafafa; }
        .purchases-table td { padding: 14px 16px; font-size: 13px; color: #333; border-bottom: 1px solid #f5f5f5; vertical-align: middle; }
        .purchases-table tr:last-child td { border-bottom: none; }
        .purchases-table tr:hover td { background: #fafafa; }

        .avatar { width: 32px; height: 32px; border-radius: 50%; background: #ffe0a9; display: inline-flex; align-items: center; justify-content: center; font-size: 11px; font-weight: bold; color: #555; margin-right: 8px; vertical-align: middle; }

        .badge { font-size: 10px; padding: 4px 10px; border-radius: 4px; font-weight: bold; }
        ...
        .gold-btn { padding: 10px 22px; background: #ffe0a9; border: none; border-radius: 15px 0 15px 0; font-size: 13px; font-weight: bold; color: #555; cursor: pointer; text-decoration: none; display: inline-block; transition: background 0.2s; }
...
<div class="main">
    <div class="topbar">
        <div>
            ...
            <div class="topbar-sub">{{ $purchases->count() }} item{{ $purchases->count() !== 1 ? 's' : '' }} bought</div>
        </div>
        <a href="{{ url('/store') }}" class="gold-btn">Browse Store</a>
    </div>
        ...
        <div class="card">
            @if($purchases->isEmpty())
                <div class="empty-state">
                    ...
                    <p>Items you buy from the store will appear here.</p>
                    <a href="{{ url('/store') }}" class="gold-btn">Browse Store</a>
                </div>
            @else
            <table class="purchases-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="color:#999; font-size:12px;">#{{ str_pad($purchase->id, 4, '0', STR_PAD_LEFT) }}</td>
                        <td><strong>{{ $purchase->item_name }}</strong></td>
                        <td>
                            <span class="avatar">{{ strtoupper(substr($purchase->seller->username, 0, 1)) }}</span>
                            {{ $purchase->seller->username }}
                        </td>
                        <td><strong>${{ number_format($purchase->price, 2) }}</strong></td>
                        <td style="color:#999; font-size:12px;">{{ $purchase->created_at->format('M j, Y') }}</td>
                        <td>
                            <span class="badge badge-{{ strtolower($purchase->status) }}">{{ $purchase->status }}</span>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
